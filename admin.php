...
<body class="admin-page">
  <div class="admin-shell">
    ...
    <main class="admin-main">
      ...
      <?php if (isset($_GET['admin_acesso'])): ?>
        <p style="color: green; font-weight: bold;">
          <?= $_GET['admin_acesso'] === 'aprovado' ? 'Acesso administrativo aprovado.' : 'Acesso administrativo recusado.' ?>
        </p>
      <?php endif; ?>
      ...
      <?php if ((int) ($_SESSION['admin_id'] ?? 0) === 1): ?>
        <section class="admin-section" id="acessos-admin">
          <div class="admin-section__header">
            <div>
              <span class="label">Acessos</span>
              <h2 class="admin-section__title">Solicitacoes administrativas</h2>
            </div>
          </div>

          <?php if (!$adminsPendentes): ?>
            <p class="admin-empty">Nenhuma conta administrativa aguardando aprovacao.</p>
          <?php endif; ?>

          <div class="request-grid">
            <?php foreach ($adminsPendentes as $adminPendente): ?>
              <article class="request-card">
                <span class="status status--waiting">Pendente</span>
                <h3><?= e($adminPendente['nome']) ?></h3>
                <div class="request-card__meta">
                  <span>E-mail: <?= e($adminPendente['email']) ?></span>
                  <span>Telefone: <?= e($adminPendente['telefone'] ?: 'Nao informado') ?></span>
                  <span>Solicitado em: <?= e(formatDateBr($adminPendente['created_at'] ?? null)) ?></span>
                </div>

                <div class="request-card__actions request-actions-compact">
                  <form action="actions/avaliar-admin.php" method="post">
                    <input type="hidden" name="admin_id" value="<?= e($adminPendente['id']) ?>">
                    <input type="hidden" name="decisao" value="aprovar">
                    <button class="mini-action mini-action--success" type="submit">Aprovar</button>
                  </form>

                  <form action="actions/avaliar-admin.php" method="post" onsubmit="return confirm('Recusar este acesso administrativo?');">
                    <input type="hidden" name="admin_id" value="<?= e($adminPendente['id']) ?>">
                    <input type="hidden" name="decisao" value="recusar">
                    <button class="mini-action" type="submit">Recusar</button>
                  </form>
                </div>
              </article>
            <?php endforeach; ?>
