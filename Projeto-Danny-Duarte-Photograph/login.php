...
<body class="auth-page">
  <main class="auth">
    <section class="auth__panel">
      ...
      <span class="label">Area administrativa</span>
      <h1 class="auth__title">Entrar na conta.</h1>
      <p class="auth__text">Acesso reservado para gerenciar agenda, clientes atendidos e solicitacoes de atendimento.</p>

      <?php if (isset($_GET['cadastro']) && $_GET['cadastro'] === 'pendente'): ?>
        <p class="auth-alert auth-alert--success">Solicitacao enviada. Aguarde a Danny aprovar seu acesso administrativo.</p>
      <?php endif; ?>

      <?php if (isset($_GET['senha']) && $_GET['senha'] === 'alterada'): ?>
        <p class="auth-alert auth-alert--success">Senha alterada com sucesso. Entre com a nova senha.</p>
      <?php endif; ?>
        ...
        <p class="auth-alert auth-alert--error">
          <?php
            echo match ($_GET['erro']) {
              ...
              'pendente' => 'Seu acesso administrativo ainda esta aguardando aprovacao.',
      ...
      <p class="auth__switch">Ainda nao tem conta? <a href="criar-conta.php">Criar conta</a></p>
    </section>
  </main>
</body>
</html>
